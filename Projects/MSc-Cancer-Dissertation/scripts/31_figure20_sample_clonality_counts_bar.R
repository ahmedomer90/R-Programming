...
# 31_figure20_sample_clonality_counts_bar.R
# Purpose: reproduce Dissertation Figure 20 as a stacked bar chart showing
...
# -----------------------------------------------------------------------------
# Preserve the validated sample order
# -----------------------------------------------------------------------------
...
# Figure 20 compares clonal and subclonal LOH fragments only.
# The rare "none" records are excluded because they arose from strict
# interval-overlap edge cases rather than a biological clonality class.

figure_20_plot_data <- sample_clonality_summary %>%
  dplyr::filter(
    clonality %in% c(
      "clonal",
      "subclonal"
...
if (any(is.na(figure_20_plot_data$count))) {
  stop(
    "Missing fragment counts were produced in the plotting data."
  )
}

if (any(figure_20_plot_data$count < 0)) {
  stop(
...
figure_20 <- ggplot(
  ...
  labs(
    title = paste(
      "number of fragments in each LOH sample",
      "and its clonality proportion"
    ),
    x = "sample",
...
sample_count_summary <- figure_20_plot_data %>%
...
figure_20_validation <- tibble::tibble(
  number_of_samples =
    dplyr::n_distinct(
      figure_20_plot_data$sample
    ),
  
  number_of_clonality_classes =
    dplyr::n_distinct(
      figure_20_plot_data$clonality
    ),
  
  clonal_LOH_fragment_count =
    sum(
      figure_20_plot_data$count[
        figure_20_plot_data$clonality ==
          "clonal"
      ]
    ),
  ...
  negative_fragment_counts =
    sum(
      figure_20_plot_data$count < 0
    )
)

print(
  figure_20_validation,
  n = Inf,
  width = Inf
)
...
if (!all(figure_file_validation$file_exists)) {
  warning(
    "One or more Figure 20 output files were not created."
  )
}
...
# Figure legend:
#
# Figure 20. Stacked bar chart showing the total number of clonal and
# subclonal LOH fragments within each tumour-region sample from the combined
# LUAD and LUSC cohort.
#
# Red represents subclonal LOH and blue represents clonal LOH. Rare records
# classified as "none" because of strict interval-overlap edge cases are
# excluded from this biological comparison.
# -----------------------------------------------------------------------------