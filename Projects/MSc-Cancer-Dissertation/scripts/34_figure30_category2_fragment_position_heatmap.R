...
# 34_figure30_category2_fragment_position_heatmap.R
#
# Purpose:
# Reproduce Dissertation Figure 30 as a chromosome-position heatmap showing
# the number of category 2 LOH fragments overlapping genomic intervals.
#
# Category 2 represents the second LOH category defined in the reconstructed
# LOH classification workflow.
#
# Each chromosome is divided into 5-Mb genomic intervals. The number of
# category 2 fragments overlapping each interval is calculated across all
...
if (!"2" %in% available_categories) {
  stop(
    "The expected LOH category '2' was not found. ",
...
# -----------------------------------------------------------------------------
# Filter category 2 LOH fragments
# -----------------------------------------------------------------------------

category2_segments <- loh_segments %>%
  dplyr::filter(
...
if (nrow(category2_segments) == 0) {
  stop(
    "No category 2 LOH fragments were found."
  )
}
...
coordinate_validation <- category2_segments %>%
  dplyr::left_join(
    chromosome_lengths,
    by = "chr"
  )
...
if (any(is.na(coordinate_validation$chromosome_length_bp))) {
  stop(
    "One or more category 2 fragments could not be matched to a chromosome."
  )
}
...
if (nrow(fragments_beyond_chromosome) > 0) {
  warning(
    ...
    " category 2 fragment(s) extend beyond the listed GRCh37 chromosome ",
    "length. Their end coordinates will be limited to the chromosome length."
  )
}

category2_segments <- coordinate_validation %>%
  dplyr::mutate(
    startpos = pmax(
      startpos,
      0
...
# -----------------------------------------------------------------------------
# Assign every category 2 fragment to all genomic bins it overlaps
# -----------------------------------------------------------------------------

category2_fragment_bins <- category2_segments %>%
  dplyr::mutate(
    start_bin = floor(
      pmax(
        startpos - 1,
...
# -----------------------------------------------------------------------------
# Count category 2 fragments overlapping each chromosome-position bin
# -----------------------------------------------------------------------------

category2_counts_by_bin <- category2_fragment_bins %>%
  dplyr::count(
    chr,
    bin_index,
    name = "category2_fragment_count"
  )
...
figure_30_all_bins <- chromosome_bins %>%
  dplyr::left_join(
    category2_counts_by_bin,
    by = c(
      "chr",
      "bin_index"
    )
...
# The dissertation figure displays only genomic bins containing at least
# one category 2 fragment. Bins with zero fragments therefore appear blank.

figure_30_plot_data <- figure_30_all_bins %>%
  dplyr::filter(
    category2_fragment_count > 0
  )

if (nrow(figure_30_plot_data) == 0) {
  stop(
    "No chromosome-position bins contained category 2 fragments."
  )
}
...
maximum_category2_count <- max(
  figure_30_plot_data$category2_fragment_count
)
...
legend_breaks <- legend_breaks[
  legend_breaks <= maximum_category2_count
]
...
figure_30 <- ggplot(
  ...
  scale_fill_gradient(
    low = "#ADD8E6",
    high = "#191970",
    name = "number of category 2 fragments",
  ...
  labs(
    title = paste(
      "number of category 2 fragments in the genome",
...
# Display Figure 30 in the RStudio Plots pane
...
chromosome_category2_summary <- figure_30_all_bins %>%
  dplyr::group_by(
    chr
  ) %>%
  dplyr::summarise(
    ...
    number_of_category2_fragments =
      sum(
        category2_segments$chr ==
          dplyr::first(chr)
      ),
    ...
    maximum_category2_fragments_in_bin =
      max(
        category2_fragment_count
      ),
    
    sum_of_category2_bin_overlap_counts =
      sum(
        category2_fragment_count
      ),
...
# -----------------------------------------------------------------------------
# Overall Figure 30 validation
# -----------------------------------------------------------------------------

figure_30_validation <- tibble::tibble(
  ...
  number_of_chromosomes_in_category2_source =
    dplyr::n_distinct(
      category2_segments$chr
    ),
  ...
  bins_with_at_least_one_category2_fragment =
    nrow(
      figure_30_plot_data
    ),
  
  maximum_category2_fragments_in_one_bin =
    max(
      figure_30_plot_data$
        category2_fragment_count
    ),
  
  minimum_positive_category2_fragments_in_one_bin =
    min(
      figure_30_plot_data$
        category2_fragment_count
    ),
  
  chromosomes_with_plotted_category2 =
    dplyr::n_distinct(
      figure_30_plot_data$chr
    ),
...
print(
  figure_30_validation,
  n = Inf,
  width = Inf
)

print(
  chromosome_category2_summary,
  n = Inf,
  width = Inf
)
...
if (!all(figure_file_validation$file_exists)) {
  warning(
    "One or more Figure 30 output files were not created."
  )
}
...
if (
  ...
  warning(
    "One or more Figure 30 output files have a zero file size."
  )
}

# -----------------------------------------------------------------------------
# Optional validation: category counts in the complete LOH dataset
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
#
# Figure 30. Chromosome-position heatmap conveying the total number of
# category 2 LOH fragments overlapping each genomic region of each chromosome
# across all patients and tumour-region samples.
#
# Each chromosome was divided into 5-Mb genomic intervals. Lighter blue
# intervals indicate fewer overlapping category 2 fragments, while darker
# blue intervals indicate genomic positions affected by a larger number of
# category 2 fragments. Blank regions indicate genomic bins with no category
# 2 LOH fragments.
# -----------------------------------------------------------------------------