...
# 35_figure31_category3_fragment_position_heatmap.R
#
# Purpose:
# Reproduce Dissertation Figure 31 as a chromosome-position heatmap showing
# the number of category 3 LOH fragments overlapping genomic intervals.
#
# Each chromosome is divided into 5-Mb genomic intervals. The script counts
# the number of category 3 fragments overlapping each interval across all
...
if (!"3" %in% available_categories) {
  stop(
    "The expected LOH category '3' was not found. ",
...
# -----------------------------------------------------------------------------
# Filter category 3 LOH fragments
# -----------------------------------------------------------------------------

category3_segments <- loh_segments %>%
  dplyr::filter(
    as.character(category) == "3"
  ) %>%
  dplyr::transmute(
...
if (nrow(category3_segments) == 0) {
  stop(
    "No category 3 LOH fragments were found."
  )
}
...
coordinate_validation <- category3_segments %>%
  dplyr::left_join(
    chromosome_lengths,
    by = "chr"
  )
...
if (any(is.na(coordinate_validation$chromosome_length_bp))) {
  stop(
    paste(
      "One or more category 3 fragments could not",
...
if (nrow(fragments_beyond_chromosome) > 0) {
  warning(
    ...
    " category 3 fragment(s) extend beyond the listed GRCh37 chromosome ",
    "length. Their end coordinates will be limited to the chromosome length."
  )
}

category3_segments <- coordinate_validation %>%
  dplyr::mutate(
    startpos = pmax(
      startpos,
      0
...
# -----------------------------------------------------------------------------
# Assign every category 3 fragment to all genomic bins it overlaps
# -----------------------------------------------------------------------------

category3_fragment_bins <- category3_segments %>%
  dplyr::mutate(
    start_bin = floor(
      pmax(
        startpos - 1,
...
# -----------------------------------------------------------------------------
# Count category 3 fragments overlapping each chromosome-position bin
# -----------------------------------------------------------------------------

category3_counts_by_bin <- category3_fragment_bins %>%
  dplyr::count(
    chr,
    bin_index,
    name = "category3_fragment_count"
  )
...
figure_31_all_bins <- chromosome_bins %>%
  dplyr::left_join(
    category3_counts_by_bin,
    by = c(
      "chr",
      "bin_index"
    )
...
# Only bins containing at least one category 3 fragment are drawn.
# Bins with zero fragments therefore appear blank.

figure_31_plot_data <- figure_31_all_bins %>%
  dplyr::filter(
    category3_fragment_count > 0
  )

if (nrow(figure_31_plot_data) == 0) {
  stop(
    "No chromosome-position bins contained category 3 fragments."
  )
}
...
maximum_category3_count <- max(
  figure_31_plot_data$category3_fragment_count
)

# The original dissertation figure displays legend labels around 50 and 100.
...
legend_breaks <- legend_breaks[
  legend_breaks <= maximum_category3_count
]
...
figure_31 <- ggplot(
  ...
  scale_fill_gradient(
    low = "#ADD8E6",
    high = "#191970",
    name = "number of category 3 fragments",
  ...
  labs(
    title = paste(
      "number of category 3 fragments in the genome",
...
# Display Figure 31 in the RStudio Plots pane.
...
# -----------------------------------------------------------------------------
# Count the original category 3 fragments on each chromosome
# -----------------------------------------------------------------------------

category3_fragments_by_chromosome <- category3_segments %>%
  dplyr::count(
    chr,
    name = "number_of_category3_fragments"
  )
...
chromosome_category3_summary <- figure_31_all_bins %>%
  dplyr::group_by(
    chr
  ) %>%
  dplyr::summarise(
    ...
    maximum_category3_fragments_in_bin =
      max(
        category3_fragment_count
      ),
    
    sum_of_category3_bin_overlap_counts =
      sum(
        category3_fragment_count
      ),
  ...
  dplyr::left_join(
    category3_fragments_by_chromosome,
    by = "chr"
  ) %>%
  dplyr::mutate(
    number_of_category3_fragments =
      tidyr::replace_na(
        number_of_category3_fragments,
        0L
      )
...
# -----------------------------------------------------------------------------
# Overall Figure 31 validation
# -----------------------------------------------------------------------------

figure_31_validation <- tibble::tibble(
  ...
  number_of_chromosomes_in_category3_source =
    dplyr::n_distinct(
      category3_segments$chr
    ),
  ...
  bins_with_at_least_one_category3_fragment =
    nrow(
      figure_31_plot_data
    ),
  
  maximum_category3_fragments_in_one_bin =
    max(
      figure_31_plot_data$
        category3_fragment_count
    ),
  
  minimum_positive_category3_fragments_in_one_bin =
    min(
      figure_31_plot_data$
        category3_fragment_count
    ),
  
  chromosomes_with_plotted_category3 =
    dplyr::n_distinct(
      figure_31_plot_data$chr
    ),
...
print(
  figure_31_validation,
  n = Inf,
  width = Inf
)

print(
  chromosome_category3_summary,
  n = Inf,
  width = Inf
)
...
if (!all(figure_file_validation$file_exists)) {
  warning(
    "One or more Figure 31 output files were not created."
  )
}
...
if (
  ...
  warning(
    "One or more Figure 31 output files have a zero file size."
  )
}
...
# -----------------------------------------------------------------------------
#
# Figure 31. Chromosome-position heatmap conveying the total number of
# category 3 LOH fragments overlapping each genomic region of each chromosome
# across all patients and tumour-region samples.
#
# Each chromosome was divided into 5-Mb genomic intervals. Lighter blue
# intervals indicate fewer overlapping category 3 fragments, while darker
# blue intervals indicate genomic positions affected by a larger number of
# category 3 fragments. Blank regions indicate genomic bins containing no
# category 3 LOH fragments.
# -----------------------------------------------------------------------------