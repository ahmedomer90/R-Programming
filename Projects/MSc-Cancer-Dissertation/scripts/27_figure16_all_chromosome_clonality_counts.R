...
# 27_figure16_all_chromosome_clonality_counts.R
# Purpose: reproduce Dissertation Figure 16 as a stacked area chart showing
...
figure_16 <- ggplot(
  ...
  labs(
    title = paste(
      "number of fragments with a particular clonality",
      "in each chromosome"
    ),
    x = "chr",
...
figure_16_validation <- tibble::tibble(
  number_of_chromosomes =
    dplyr::n_distinct(
      figure_16_plot_data$chr
    ),
  
  number_of_clonality_classes =
    dplyr::n_distinct(
      figure_16_plot_data$clonality
    ),
  
  clonal_LOH_fragment_count =
    sum(
      figure_16_plot_data$count[
        figure_16_plot_data$clonality ==
          "Clonal LOH"
      ]
    ),
  ...
  negative_fragment_counts =
    sum(
      figure_16_plot_data$count < 0
    )
)

print(
  figure_16_validation,
  n = Inf,
  width = Inf
)
...
if (!all(figure_file_validation$file_exists)) {
  warning(
    "One or more Figure 16 output files were not created."
  )
}
...
# Figure legend:
#
# Figure 16. Area chart showing the number of fragments on each chromosome
...
# having no LOH, and salmon-red represents clonal LOH.
#
