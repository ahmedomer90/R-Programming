...
# 37_figure33_category5_fragment_position_heatmap.R
#
# Purpose:
# Reproduce Dissertation Figure 33 as a chromosome-position heatmap showing
# the number of category 5 LOH fragments overlapping genomic intervals.
#
# Each chromosome is divided into 5-Mb genomic intervals. The script counts
# the number of category 5 fragments overlapping each interval across all
...
if (!"5" %in% available_categories) {
  stop(
    "The expected LOH category '5' was not found. ",
...
# -----------------------------------------------------------------------------
# Filter category 5 LOH fragments
# -----------------------------------------------------------------------------

category5_segments <- loh_segments %>%
  dplyr::filter(
    as.character(category) == "5"
  ) %>%
  dplyr::transmute(
...
if (nrow(category5_segments) == 0) {
  stop(
    "No category 5 LOH fragments were found."
  )
}
...
coordinate_validation <- category5_segments %>%
  dplyr::left_join(
    chromosome_lengths,
    by = "chr"
  )
...
if (any(is.na(coordinate_validation$chromosome_length_bp))) {
  stop(
    paste(
      "One or more category 5 fragments could not",
...
if (nrow(fragments_beyond_chromosome) > 0) {
  warning(
    ...
    " category 5 fragment(s) extend beyond the listed GRCh37 chromosome ",
    "length. Their end coordinates will be limited to the chromosome length."
  )
}

category5_segments <- coordinate_validation %>%
  dplyr::mutate(
    startpos = pmax(
      startpos,
      0
...
# -----------------------------------------------------------------------------
# Assign every category 5 fragment to all genomic bins it overlaps
# -----------------------------------------------------------------------------

category5_fragment_bins <- category5_segments %>%
  dplyr::mutate(
    start_bin = floor(
      pmax(
        startpos - 1,
...
# -----------------------------------------------------------------------------
# Count category 5 fragments overlapping each chromosome-position bin
# -----------------------------------------------------------------------------

category5_counts_by_bin <- category5_fragment_bins %>%
  dplyr::count(
    chr,
    bin_index,
    name = "category5_fragment_count"
  )
...
figure_33_all_bins <- chromosome_bins %>%
  dplyr::left_join(
    category5_counts_by_bin,
    by = c(
      "chr",
      "bin_index"
    )
...
# Only bins containing at least one category 5 fragment are drawn.
# Bins with zero fragments therefore appear blank.

figure_33_plot_data <- figure_33_all_bins %>%
  dplyr::filter(
    category5_fragment_count > 0
  )

if (nrow(figure_33_plot_data) == 0) {
  stop(
    "No chromosome-position bins contained category 5 fragments."
  )
}
...
maximum_category5_count <- max(
  figure_33_plot_data$category5_fragment_count
)

# The original dissertation Figure 33 displays legend labels around
# 10, 20 and 30 fragments.
...
legend_breaks <- legend_breaks[
  legend_breaks <= maximum_category5_count
]
...
figure_33 <- ggplot(
  ...
  scale_fill_gradient(
    low = "#ADD8E6",
    high = "#191970",
    name = "number of category 5 fragments",
  ...
  labs(
    title = paste(
      "number of category 5 fragments in the genome",
...
# Display Figure 33 in the RStudio Plots pane.
...
# -----------------------------------------------------------------------------
# Count original category 5 fragments on each chromosome
# -----------------------------------------------------------------------------

category5_fragments_by_chromosome <- category5_segments %>%
  dplyr::count(
    chr,
    name = "number_of_category5_fragments"
  ) %>%
  tibble::as_tibble()
...
chromosome_category5_summary <- figure_33_all_bins %>%
  dplyr::group_by(
    chr
  ) %>%
  dplyr::summarise(
    ...
    maximum_category5_fragments_in_bin =
      max(
        category5_fragment_count
      ),
    
    sum_of_category5_bin_overlap_counts =
      sum(
        category5_fragment_count
      ),
  ...
  dplyr::left_join(
    category5_fragments_by_chromosome,
    by = "chr"
  ) %>%
  dplyr::mutate(
    number_of_category5_fragments =
      tidyr::replace_na(
        number_of_category5_fragments,
        0L
      )
...
# -----------------------------------------------------------------------------
# Overall Figure 33 validation
# -----------------------------------------------------------------------------

figure_33_validation <- tibble::tibble(
  ...
  number_of_chromosomes_in_category5_source =
    dplyr::n_distinct(
      category5_segments$chr
    ),
  ...
  bins_with_at_least_one_category5_fragment =
    nrow(
      figure_33_plot_data
    ),
  
  maximum_category5_fragments_in_one_bin =
    max(
      figure_33_plot_data$
        category5_fragment_count
    ),
  
  minimum_positive_category5_fragments_in_one_bin =
    min(
      figure_33_plot_data$
        category5_fragment_count
    ),
  
  chromosomes_with_plotted_category5 =
    dplyr::n_distinct(
      figure_33_plot_data$chr
    ),
...
print(
  figure_33_validation,
  n = Inf,
  width = Inf
)

print(
  chromosome_category5_summary,
  n = Inf,
  width = Inf
)
...
if (!all(figure_file_validation$file_exists)) {
  warning(
    "One or more Figure 33 output files were not created."
  )
}
...
if (
  ...
  warning(
    "One or more Figure 33 output files have a zero file size."
  )
}
...
# -----------------------------------------------------------------------------
#
# Figure 33. Chromosome-position heatmap conveying the total number of
# category 5 LOH fragments overlapping each genomic region of each chromosome
# across all patients and tumour-region samples.
#
# Each chromosome was divided into 5-Mb genomic intervals. Lighter blue
# intervals indicate fewer overlapping category 5 fragments, while darker
# blue intervals indicate genomic positions affected by a larger number of
# category 5 fragments. Blank regions indicate genomic bins containing no
# category 5 LOH fragments.
# -----------------------------------------------------------------------------