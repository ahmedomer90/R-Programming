...
# M-values are generated only for the selected SHH_1-versus-Other samples below.
...
labels <- ifelse(labels == "SHH_1", "SHH_1", "Other")
table(labels)
...
# Set the reference level explicitly so coef = 2 tests SHH_1 versus Other.
class.grp <- factor(
  labels[keep],
...
stopifnot(ncol(class.Ms) == length(class.grp))
...
stopifnot(
  ncol(design) == 2L,
  identical(colnames(design), c("(Intercept)", paste0("class.grp", class1)))
)
...
{ 
  ...
  # If more than one DMR maps to the same gene symbol, retain the first match.
...
# Removed: geneNames[336] <- "NA"
# A fixed position is not reliable across different analysis outputs. 
