...
# M-values are generated only for the selected SHH_3-versus-Other samples below.
...
labels <- ifelse(labels == "SHH_3", "SHH_3", "Other")
table(labels)
...
# With this ordering, coefficient 2 in the design matrix is SHH_3 versus Other.
class.grp <- factor(
  labels[keep],
...
## Because levels(class.grp) is c(class2, class1), coef = 2 tests SHH_3 versus Other.
design <- model.matrix(~ class.grp)

stopifnot(
  ncol(design) == 2L,
  identical(levels(class.grp), c("Other", "SHH_3"))
)
