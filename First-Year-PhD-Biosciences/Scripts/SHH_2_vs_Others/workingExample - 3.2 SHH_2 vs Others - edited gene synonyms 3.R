...
# Update older gene symbols to current HGNC-approved symbols

MB_expression_data$HGNC_symbol_from_ensemblv77[
  MB_expression_data$EnsemblGeneID_from_ensemblv77 == "ENSG00000177096" &
    MB_expression_data$HGNC_symbol_from_ensemblv77 == "FAM109B"
] <- "PHETA2"

MB_expression_data$HGNC_symbol_from_ensemblv77[
  MB_expression_data$EnsemblGeneID_from_ensemblv77 == "ENSG00000168070" &
    MB_expression_data$HGNC_symbol_from_ensemblv77 == "C11orf85"
] <- "MAJIN"

MB_expression_data$HGNC_symbol_from_ensemblv77[
  MB_expression_data$EnsemblGeneID_from_ensemblv77 == "ENSG00000162618" &
    MB_expression_data$HGNC_symbol_from_ensemblv77 == "ELTD1"
] <- "ADGRL4"

MB_expression_data$HGNC_symbol_from_ensemblv77[
  MB_expression_data$EnsemblGeneID_from_ensemblv77 == "ENSG00000178927" &
    MB_expression_data$HGNC_symbol_from_ensemblv77 == "C17orf62"
] <- "CYBC1"

MB_expression_data$HGNC_symbol_from_ensemblv77[
  MB_expression_data$EnsemblGeneID_from_ensemblv77 == "ENSG00000133247" &
    MB_expression_data$HGNC_symbol_from_ensemblv77 == "SUV420H2"
] <- "KMT5C"

MB_expression_data$HGNC_symbol_from_ensemblv77[
  MB_expression_data$EnsemblGeneID_from_ensemblv77 == "ENSG00000135390" &
    MB_expression_data$HGNC_symbol_from_ensemblv77 == "ATP5G2"
] <- "ATP5MC2"
...
MB_expression_data_class <- annot$SimplifiedGrps

MB_expression_data2 = as.matrix(MB_expression_data2)

labels = annot$SimplifiedGrps

# MEMORY-SAVING CHANGE:
# Do not create full beta- and M-value matrices for all 763 samples here.
# They would occupy several gigabytes each and create additional copies later.
# M-values are generated only for the selected SHH_2-versus-Other samples below.
...
labels <- ifelse(labels == "SHH_2", "SHH_2", "Other")
table(labels)
...
## MEMORY-SAVING CHANGE: subset the GenomicRatioSet first, then calculate
## M-values only for the selected samples.
keep <- labels %in% c(tmpClass1, tmpClass2)

stopifnot(
  length(labels) == ncol(MB_methylation_data),
  all(keep)
)

class.GM <- MB_methylation_data[, keep]

# Set the reference and comparison levels explicitly.
# With this ordering, coefficient 2 in the design matrix is SHH_2 versus Other.
class.grp <- factor(
  labels[keep],
  levels = c(class2, class1)
)

# Reproduce the original M-value calculation exactly,
# but process the probes in chunks to reduce peak memory usage.
n_probes  <- nrow(class.GM)
n_samples <- ncol(class.GM)
chunk_size <- 25000L
...
class.Ms <- matrix(
  ...
  nrow = n_probes,
  ncol = n_samples,
  dimnames = list(rownames(class.GM), colnames(class.GM))
)

for (first_row in seq.int(1L, n_probes, by = chunk_size)) {
  
  last_row <- min(first_row + chunk_size - 1L, n_probes)
  rows <- first_row:last_row
  
  beta_chunk <- minfi::getBeta(class.GM[rows, ])
  
  # Exact boundary handling used by the original script
  beta_chunk[beta_chunk == 0] <- 0.000001
  beta_chunk[beta_chunk == 1] <- 0.999999
  
  # Exact transformation used by the original script
  class.Ms[rows, ] <- logit2(beta_chunk)
...
stopifnot(
  identical(dim(class.Ms), c(nrow(class.GM), ncol(class.GM))),
  identical(rownames(class.Ms), rownames(class.GM)),
  identical(colnames(class.Ms), colnames(class.GM))
)

stopifnot(
  ncol(class.Ms) == length(class.grp),
  identical(levels(class.grp), c(class2, class1))
)
print(table(class.grp))

## Set up design matrix.
## Because levels(class.grp) is c(class2, class1), coef = 2 tests SHH_2 versus Other.
design <- model.matrix(~ class.grp)

stopifnot(
  ncol(design) == 2L,
  identical(levels(class.grp), c("Other", "SHH_2"))
)
...
openxlsx::write.xlsx(
  ...
  file = file.path(
    subDir,
    paste0(names(table(labels))[2], "_nearestGeneInfo.xlsx")
  ),
  rowNames = FALSE
)
...
seqlevels(dmr.gr3)
class.GM <- GenomeInfoDb::keepSeqlevels(
  class.GM,
...
dmr.gr3 <- GenomeInfoDb::keepSeqlevels(
  dmr.gr3,
  c(paste0("chr", 1:22), "chrX"),
  pruning.mode = "coarse"
)
...
{ 
  ...
  results[[i]] <- maxDiffFinder(i, gord.GM2 = class.GM, dmr.gr2 = dmr.gr3, labels=class1) 
} 

##Get maximal average across these samples 
resMatrix <- matrix(ncol=length(dmr.gr3), nrow=ncol(class.GM)) 
for(i in 1:length(dmr.gr3)) 
{ 
  ## Get region 
...
rownames(resMatrix) <- colnames(class.GM) 
...
{ 
  ...
  if (length(colNumber) == 0L) {
    next
  }
  colNumber <- colNumber[1L] 
  
  CpGs <- names(results[[colNumber]]$meth) 
  
  ## Get beta values only for these CpGs across all samples.
  ## This avoids keeping a full 485,512 x 763 beta matrix in memory.
  probe.index <- match(CpGs, rownames(MB_methylation_data))

  stopifnot(
    length(probe.index) == length(CpGs),
    !anyNA(probe.index)
  )

  all.CpGs <- minfi::getBeta(
    MB_methylation_data[probe.index, ]
  )

  all.CpGs <- all.CpGs[
    results[[colNumber]]$sel,
    ,
    drop = FALSE
  ]

  all.CpGs <- colMeans(all.CpGs)
  ...
  if(length(toTest) == 0) { cat(paste0("No Exp for Gene ", yfg,"\n"))} else { 
    ...
    { 
      ...
      par(cex.axis=1) 
      par(cex.axis=0.6) 
      expression.levels <- levels(factor(MB_expression_data_class))
      
      axis(
        1,
        at = seq_along(expression.levels),
        labels = expression.levels,
        las = 2,
        cex.axis = 0.6
      ) 
...
# Removed hard-coded geneNames[336] override; missing symbols are skipped automatically.
