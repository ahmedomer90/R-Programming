...
class.grp <- droplevels(factor(labels[keep]))
...
{ 
  ...
  results[[i]] <- maxDiffFinder(i, gord.GM2 = class.GM, dmr.gr2 = dmr.gr3, labels="SHH_2") 
} 
...
{ 
  ...
  probe.index <- probe.index[!is.na(probe.index)]
  all.CpGs <- minfi::getBeta(MB_methylation_data[probe.index, ])
  all.CpGs <- all.CpGs[results[[colNumber]]$sel, , drop = FALSE]
