...
# M-values are generated only for the selected SHH_3C-versus-Other samples below.
...
# With this ordering, coefficient 2 in the design matrix is SHH_3C versus Other.
class.grp <- factor(
  labels[keep],
...
## Because levels(class.grp) is c(class2, class1), coef = 2 tests SHH_3C versus Other.
design <- model.matrix(~ class.grp)

stopifnot(
  ncol(design) == 2L,
  identical(levels(class.grp), c("Other", "SHH_3C"))
)
