...
# M-values are generated only for the selected SHH_3A-versus-Other samples below.
...
labels <- ifelse(labels == "SHH_3A", "SHH_3A", "Other")
table(labels)
...
# With this ordering, coefficient 2 in the design matrix is SHH_3A versus Other.
class.grp <- factor(
  labels[keep],
...
## Because levels(class.grp) is c(class2, class1), coef = 2 tests SHH_3A versus Other.
design <- model.matrix(~ class.grp)

stopifnot(
  ncol(design) == 2L,
  identical(levels(class.grp), c("Other", "SHH_3A"))
)
